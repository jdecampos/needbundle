...
$view['slots']->set('mauticContent', 'need');

$objectId = $form->vars['data']->getId();
if (!empty($objectId)) {
  $name   = $form->vars['data']->getName();
  $header = $view['translator']->trans('kompulse.need.header.edit', ['%name%' => $name]);
} else {
  $header = $view['translator']->trans('kompulse.need.header.new');
}
